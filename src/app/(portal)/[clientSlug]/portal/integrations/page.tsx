...
import { RecipeCard } from "@/components/integrations/recipe-card";
import { RecipeSetupModal } from "@/components/integrations/recipe-setup-modal";
import { IntegrationRequestModal } from "@/components/integrations/integration-request-modal";
import { ActiveAutomationCard } from "@/components/integrations/active-automation-card";
...
function PortalAutomationsContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [recipesRes, automationsRes, connectionsRes, requestsRes] = await Promise.all([
        fetch("/api/integrations/recipes"),
        fetch("/api/integrations/client"),
  ...
  async function handleSetup(recipeId: string, config: Record<string, unknown>) {
    setSaving(true);
    try {
      const res = await fetch("/api/integrations/client", {
        method: "POST",
  ...
  async function handleEditSave(recipeId: string, config: Record<string, unknown>) {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/integrations/client/${editingAutomation.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleToggle(automationId: string, enabled: boolean) {
    ...
    try {
      const res = await fetch(`/api/integrations/client/${automationId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
